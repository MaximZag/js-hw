...
for (const simpson of simpsons) {
    ...
    Object.entries(simpson).forEach(([key, value]) => {
        let subdiv = document.createElement('div');
        subdiv.innerText = `${key}: ${value}`;
...
for (const coursesArrayElement of coursesArray) {
    ...
    Object.entries(coursesArrayElement).forEach(([key, value]) => {
        if (key === 'modules') {
            let bigBlock = document.createElement('div');
            block.appendChild(bigBlock);
            for (const moduleElement of coursesArrayElement.modules) {
                let smallBlock = document.createElement('div');
                smallBlock.innerText = moduleElement;
                bigBlock.appendChild(smallBlock);
            }
        } else {
